...
const assetToArray = (asset: AztecAsset) => [asset.id || 0, asset.erc20Address || '0x0000000000000000000000000000000000000000', asset.assetType || 0];
...
export class DefiBridgeProxy {
  ...
  async finalise(
    ...
    const parsedLogs = receipt.logs.filter((l: any) => l.address == contract.address).map((l: any) => contract.interface.parseLog(l));
